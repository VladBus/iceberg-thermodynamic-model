...
| External data sources (ERA5/EN4/IBCAO/obs) | `docs/data_sources.md`           |
...
python python/tests/test_stage10_18d_per_iceberg.py                  # Stage 10.18D per-iceberg Enderlin23 (26 checks)
python python/analysis/stage10_18d_per_iceberg.py                    # Stage 10.18D per-iceberg analysis (10 figures + summary)
python python/tests/test_stage10_21_precision.py                     # Stage 10.21 EOS float32 precision study (13 checks)
python python/tests/test_stage10_21_convective_adjustment.py         # Stage 10.21 convective-adjustment experiments (18 checks)
python python/tests/test_stage10_22_block200_audit.py                # Stage 10.22 Block-200 stability audit (300 checks)

# Stage 10.19 production runs (iceberg connected behind env gate; default OFF)
fpm run --flag "-I/usr/include" -- stage10.19_legacy_check            # Legacy (no iceberg) — bit-identical to pre-10.19
ICEBERG_PRODUCTION=true fpm run --flag "-I/usr/include" -- stage10.19_1day_gate   # 1-day gate: ocean NaN → iceberg "steps executed: 0" (guarded)
ICEBERG_PRODUCTION=true fpm run --flag "-I/usr/include" -- stage10.19_7day_gate   # 7-day gate (same NaN diagnosis)
ICEBERG_PRODUCTION=true fpm run --flag "-I/usr/include" -- stage10.19_30day_gate  # 30-day gate (must show "steps executed: 720" after ocean-init stabilization)
```
...
Actions (`.github/workflows/ci.yml`) runs the full fpm test battery (59
...
| 10.18D                | Velocity-resolved / per-iceberg observational upgrade: **10.18C data gap CLOSED** — Enderlin23 individual dataset RETRIEVED (USAP-DC 10.15784/601679 via direct POST; JS reCAPTCHA not enforced; MD5 `821bed51660957b65a896c42fb88a057` verified); dataset `data/validation/observations/stage10.18d/enderlin23_per_iceberg.csv` (**743 per-iceberg rows**, 15 sites 2011-2022, melt+draft+geometry+coords); regional maxima reproduce paper (WAP/WAIS max 1.1-1.3x of ~50/~40 m/a; EAIS/EAP p95 5.2/3.6 vs ~5 m/a); Thwaites 24 m/a/degC consistent per-iceberg (76% of 167 bergs imply TF in 0.2-1.6 degC); **legacy exceeds 99.6% (TF=1.5) / 88% (TF=0.6) of per-iceberg observed melt** (median 14x/5.6x); velocity-dependent variants bracket observed medians; **per-iceberg C_eff_submarine median 1.56e-7 = 0.16x production**; velocity dependence NOT IDENTIFIABLE (no U_ocean) → **ADCP-equipped Schild21-style campaign designed**; **OPTION D refined + KEEP_CURRENT**; NO production physics change; `test_stage10_18d_per_iceberg.py` 26 checks + `stage10_18d_per_iceberg.py` 10 figures | `docs/validation/stage10.18d_velocity_resolved_observational_upgrade.md`                                                                                                                                                                           |
| 10.19                | Production runtime & full iceberg coupling recovery: **Stage 10.15 finding addressed** — `app/main.f90` now runs the iceberg module behind `ICEBERG_PRODUCTION=true` env gate (default OFF → legacy bit-identical); `fpm.toml` main-file fix; NaN validity guard in `get_ocean_profile`; **ocean zombie forensic (CASE C/D)**: day_00 clean → first NaN in Block 210 at III=2 of day 1 (Block 200 ~25 m/s geostrophic spike from EN4 thermal-wind init) → day_01 **56.5% NaN**, iceberg start cell (i=61,j=37) 15/18 levels NaN → **CASE C BLOCKER** with CASE D root cause (T-03 family); gates 1d/7d/30d exit 0, iceberg `steps executed: 0` (guarded); offline real-forcing path (TEST_11) remains validated; **D-17: KEEP_CURRENT physics + env-gated integration**; next = ocean-init stabilization stage, acceptance = 30-day gate with `steps executed: 720`; full battery PASS; production physics unchanged (only NaN-guard in `src/`) | `docs/validation/stage10.19_production_runtime_coupling_recovery.md`                                                                                                                                                                           |
| 10.20                | Ocean initialization & numerical stabilization — **closed necessary-but-not-sufficient (D-18)**: **data-level fix adopted** — full-coverage merged ERA5 Jan file (`era5_2020_01_fullcoverage_merged.nc`, 124 slices) → **CASE C resolved**: 3-day `ICEBERG_PRODUCTION=true` gate PASS (36 steps, 0 NaN), days 1–4 of 30-day gate clean, 10.19's `steps executed: 0` → **55**; **residual instability characterized, NOT fixed (physics)**: CA 1000-iter guard saturates from day 1 (maxiter=1001 every day; residual inversions pinned at 1.1921E-07 = 2⁻²³ float32 EOS quantization, T-01/T-03 family) → slow T/S/ρ corruption (day 4 already nonphysical: −45…+50 °C, S<0, 0 NaN) → **density-first divergence mid-day 5** (first NaN = `NaN_RO=198` at `F_after_conv` III=6; 0 NaN in U/V/T/S) → zombie from day 6 → 30-day gate `steps executed: 55`, **acceptance (360) NOT met**; **cadence correction: 12 steps/day × 30 = 360** (10.19 "720" = doc error; 3-day gate = 36 steps); default-path recommendation to point `src/param.f90:301` at fullcov file recorded, NOT applied; only source change = Stage-10.19-pending iceberg print-format fix (`app/main.f90`, diagnostics only); next = approved physics stage (EOS precision / CA threshold / guard per RULES.md, or preconditioned-solver study), acceptance = 30-day gate `steps executed: 360`; full battery PASS | `docs/validation/stage10.20_ocean_initialization_numerical_stabilization.md`                                                                                                                                                                           |
| 10.21                | CA/EOS float32-precision research audit (T-01 CLOSED): CA guard saturation = **float32 EOS quantization floor** (2⁻²³ = 1.1921E-07, NOT convective-physics instability); 6 experiments A0/EXP-A/B15/B24/C/D behind `STAGE1021_*` switches (CA_F64/MIX/SCOPE/EPS), all OFF default → bit-identical legacy; **eliminating the CA residual does NOT stabilize the ocean** (ALL closures bit-identical days 1–4, ALL diverge density-first mid-day-5; 30-day gates A0=55/A=57/B15=57/B24=51/C=56/D=52, acceptance NOT met → root cause upstream: Block-200 geostrophic spike / Block-210 Thomas solver, T-03 family); NO production physics change; D-19 KEEP_CURRENT (0.9e-7 threshold retained); `test_stage10_21_precision.py` 13 checks + `test_stage10_21_convective_adjustment.py` 18 checks; full battery PASS | `docs/validation/stage10.21_convective_precision_stabilization.md`                                                                                                                                                                           |
| 10.22                | Block-200 stability audit (causal chain ISOLATED, D-20): replay experiments a0–a4 (30 d, step 1; CSVs `stage1022_{probes,block200,block210}.csv`, per-step/30-day A5 verdicts) behind env switches `STAGE1022_DIAG`/`_FREEZE_RO`/`_FREEZE_THERMAL_WIND`/`_FREEZE_TS`/`_FREEZE_RO_DOWNSTREAM` — all default OFF → bit-identical legacy verified (daily-diagnostics md5 `1ef13cb4…`); **chain**: CA/EOS density corruption (ρ<0 day 4 s1) → **first NaN day 5 s6** (`CA_after ro_nan=198`) → **Block 200 = transmitter** (ro in, momentum NaN out: `u2_nan=333`; two-path budget split EXACT) → **Block 210 = amplifier** (rhs 1.13E16, den 7.06E-04 → day-6 zombie 142,081); **Thomas pivots negative BY CONSTRUCTION** (piv_neg = piv_cnt = 100 %, piv_min = 1.0) — structural, NOT pathology; stabilizer levers `freeze_ro`/`freeze_ts`/`freeze_ro_downstream` restore physical validity (30/30, worst rel 4.213e-6) but **DIAGNOSTIC-ONLY**; **production physics KEEP_CURRENT**; `test_stage10_22_block200_audit.py` 300 checks; no Fortran test added; full battery PASS (59 targets) | `docs/validation/stage10.22_ocean_density_thermal_wind_block200_audit.md`                                                                                                                                                                           |
