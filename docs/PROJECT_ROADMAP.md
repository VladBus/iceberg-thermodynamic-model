...
**Updated:** 2026-09-22
**Current scientific stage:** Stage 10.19 — production runtime & full iceberg coupling recovery (COMPLETED; iceberg module connected to the production executable behind `ICEBERG_PRODUCTION=true` env gate, default OFF/bit-identical; fpm.toml main-file fix; NaN validity guard in `get_ocean_profile`; 1/7/30-day production gates run — ocean zombie state (56.5 % NaN from day 1, start cell 15/18 levels NaN) classified CASE C BLOCKER with CASE D root cause (Block 200 spike → Block 210 Thomas blowup, T-03 family); production physics KEEP_CURRENT; offline real-forcing iceberg path remains validated)
**Current status:** Stages 10.14–10.18D committed and pushed (`0504853` 10.18A, `d99a4bf` 10.18B, 10.18C/10.18D pending); **Stage 10.19 (production runtime & full iceberg coupling recovery) COMPLETE** (2026-09-22): `app/main.f90` previously contained ZERO iceberg calls (Stage 10.15 finding — module compiled but unreachable from production) — now connected behind an env gate (`ICEBERG_PRODUCTION=true`, default OFF → legacy bit-identical): `iceberg_init` after `init_thermal_wind()` at the TEST_11 start position (75°N, 30°E → i=61, j=37), hourly `iceberg_step` in the III loop with live `t2/s2/u2/v2` ocean profile + ERA5 atmos + bathymetry; `fpm.toml` fixed (`main = "iceberg_main.f90"` → `main.f90`); NaN validity guard added to `get_ocean_profile` (explicit `ok=.false.` detection of the known zombie state, not masking); **gates**: 1-day / 7-day / 30-day production runs exit 0 and reproduce day_00 clean → first NaN in Block 210 at III=2 of day 1 → day_01 56.5 % NaN (142,081/251,370 cells; T clamped at 273.1 K) → start cell 15/18 levels NaN → `ICEBERG[prod] WARNING` on every candidate step, `steps executed : 0`; **CASE C BLOCKER** (online-coupled run not physically valid) with **CASE D root cause** (EN4 thermal-wind init → Block 200 ~25 m/s spike → Thomas blowup, T-03 family — NOT introduced by this stage, NOT fixable in a no-physics stage); offline real-forcing path (TEST_11 family) remains fully validated; **decision D-17**: KEEP_CURRENT physics, env-gated integration OFF by default, next stage = approved ocean-init stabilization, after which the 30-day gate must show `steps executed = 720`; full fpm battery + Python suites PASS; `git diff --check` clean; production physics unchanged (only `get_ocean_profile` NaN-guard in `src/`).
...
**Acceptance gate after stabilization:** re-run the 30-day gate with
`ICEBERG_PRODUCTION=true` — expect `steps executed = 720` and a physically
valid trajectory (no NaN warnings).
