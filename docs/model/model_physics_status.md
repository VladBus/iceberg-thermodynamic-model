...
**Production baseline:** Stage 10.6 + corrective documentation commit `40d4a3b`  
**FPM test targets:** 50  
**Local status:** 50/50 PASS; strict build clean; `git diff --check` clean
...
| Basal melt | `m_basal = gamma_T * max(T-Tf,0)/(rho_ice*Lf)` | B |
