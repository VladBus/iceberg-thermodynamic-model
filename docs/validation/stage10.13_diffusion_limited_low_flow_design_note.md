...
convection; the basal ice-ocean interface has *opposed* thermal (destabilizing)
...
*source of transfer coefficients* `γ_T`, `γ_S` in the low-flow regime.
...
5. **ε/w\*** at U=0**: buoyancy-driven w\* from DC (Keitzl w* definition) —
...
2. Parameter sweep over scientifically justified ranges only:
   T_w ∈ [1,8] °C (Arctic), S_w ∈ [30,35] g/kg, U ∈ [0,0.1] m/s, t ∈ [1h,10 d].
   No arbitrary calibration.
3. Comparison: candidate vs Stage 10.11 floor; vs 10.8.2 quiescent rows
   (10.8.2 (NJ80 rows, KW84); re-scoring only with explicit uncertainty.
4. Fortran integration deferred until acceptance criteria met.
