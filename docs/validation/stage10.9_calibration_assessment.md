...
| --- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | KW84 reproduced "within reported range" (0.70x)               | **PASS with caveat**: 0.705x at L_char=draft=20 m; in _production_ L_char = berg length 40-100 m the ratio is 0.55-0.62, i.e. _below_ the reported 0.05-0.07 band, so "better than the reported range" is overstated                                         |
...
Using production L (berg length) instead of draft moves the KW84 row _further_ from
...
_degrades_ the one field-anchored row. (Also: pressure enters only through Tf's small
