...
def analyze_heat_diagnostics(nc_dir, diag_csv, output_csv, output_txt):
    ...
    for nc_file in nc_files:
        ...
        try:
            ...
            heat_data.append({
                ...
                'humidity_mean': hum_mean, 'humidity_min': hum_min, 'humidity_max': hum_max,
                'cloud_mean': cld_mean, 'cloud_min': cld_min, 'cloud_max': cld_max,
    ...
    with open(output_txt, 'w') as f:
        ...
        f.write(f"  Mean: {heat_df['temp_mean'].mean():.3f} °C (min {heat_df['temp_min'].min():.3f}, max {heat_df['temp_max'].max():.3f})\n")
        f.write(f"  Surface Mean: {heat_df['temp_surf_mean'].mean():.3f} °C (min {heat_df['temp_surf_min'].min():.3f}, max {heat_df['temp_surf_max'].max():.3f})\n")
        
        # Salinity
        f.write(f"\nSalinity (mass fraction):\n")
        f.write(f"  Mean: {heat_df['salt_mean'].mean():.5f} (min {heat_df['salt_min'].min():.5f}, max {heat_df['salt_max'].max():.5f})\n")
        
        # Density
        f.write(f"\nDensity Anomaly (g/cm³):\n")
        f.write(f"  Mean: {heat_df['dens_mean'].mean():.5f} (min {heat_df['dens_min'].min():.5f}, max {heat_df['dens_max'].max():.5f})\n")
        
        # Velocities
        f.write(f"\nVelocities (cm/s):\n")
        f.write(f"  U max: {heat_df['u_max'].max():.2f}, V max: {heat_df['v_max'].max():.2f}, W max: {heat_df['w_max'].max():.4f}\n")
        
        # Air temperature
        f.write(f"\nAir Temperature (forcing):\n")
        f.write(f"  Mean: {heat_df['air_temp_mean'].mean():.2f} °C (min {heat_df['air_temp_min'].min():.2f}, max {heat_df['air_temp_max'].max():.2f})\n")
        
        # Humidity
        if not heat_df['humidity_mean'].isna().all():
            f.write(f"\nHumidity (relative, 0-1):\n")
            f.write(f"  Mean: {heat_df['humidity_mean'].mean():.3f} (min {heat_df['humidity_min'].min():.3f}, max {heat_df['humidity_max'].max():.3f})\n")
        
        # Cloud
        if not heat_df['cloud_mean'].isna().all():
            f.write(f"\nCloud Cover (0-1):\n")
            f.write(f"  Mean: {heat_df['cloud_mean'].mean():.3f} (min {heat_df['cloud_min'].min():.3f}, max {heat_df['cloud_max'].max():.3f})\n")
        
        # Wind
        f.write(f"\nWind (m/s):\n")
        f.write(f"  Mean: {heat_df['wind_mean'].mean():.2f}, Max: {heat_df['wind_max'].max():.2f}\n")
        ...
        if diag_cols:
            ...
            for col in diag_cols:
                if heat_df[col].notna().any():
                    f.write(f"  {col}: mean={heat_df[col].mean():.3e}, min={heat_df[col].min():.3e}, max={heat_df[col].max():.3e}\n")
        ...
        for _, row in heat_df.iterrows():
            f.write(f"\nDay {int(row['day']):2d}:\n")
            f.write(f"  T: {row['temp_mean']:.3f} °C (surf: {row['temp_surf_mean']:.3f})\n")
            ...
            f.write(f"  Wind: {row['wind_mean']:.2f} m/s, Tau: {row['tau_max']:.3f} dyn/cm²\n")
...
def main():
    parser = argparse.ArgumentParser(description="Analyze HEAT thermodynamics diagnostics")
    ...
    parser.add_argument("--output-csv", default="data/output/daily_heat_summary.csv", help="Output CSV")
    parser.add_argument("--output-txt", default="data/output/heat_report.txt", help="Output report TXT")
    parser.add_argument("--compare-on-off", action="store_true", help="Compare HEAT ON vs OFF (not yet implemented)")
    ...
    if args.compare_on_off:
        ...
        return analyze_heat_diagnostics(args.nc_dir, args.diag_csv, args.output_csv, args.output_txt)
