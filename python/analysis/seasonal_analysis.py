...
def load_manifest(manifest_path: pathlib.Path) -> dict:
    ...
    if len(files) != expected_days:
        raise ValueError(f"Manifest has {len(files)} files but expected {expected_days} days")
    ...
    for f in manifest["files"]:
        expected_date = (datetime.fromisoformat(manifest["start_date"]) + timedelta(days=f["day"] - 1)).date().isoformat()
        if f["date"] != expected_date:
            raise ValueError(f"Date mismatch for day {f['day']}: expected {expected_date}, got {f['date']}")
...
def analyze_seasonal(manifest: dict, diag_csv: pathlib.Path, 
    ...
    if 'month' not in diag.columns or diag['month'].nunique() == 1:
        diag['month'] = diag['day'].apply(lambda d: 1 if d <= 31 else (2 if d <= 59 else 3))
        diag['month_name'] = diag['month'].map({1: 'January', 2: 'February', 3: 'March'})
    ...
    for f_info in manifest["files"]:
        ...
        try:
            ...
            seasonal_data.append({
                'day': day,
                'month': month,
                'month_name': ['January', 'February', 'March'][min(2, (day - 1) // 31)],
    ...
    if seasonal_df['day'].nunique() != expected_days:
        print(f"ERROR: Duplicate days detected. Unique days: {seasonal_df['day'].nunique()}, expected: {expected_days}")
        return 1
    ...
    with open(output_txt, 'w') as f:
        ...
        if diag_cols:
            ...
            for col in diag_cols:
                if seasonal_df[col].notna().any() and pd.api.types.is_numeric_dtype(seasonal_df[col]):
        ...
        f.write(f"Missing days: {set(range(1, expected_days + 1)) - set(seasonal_df['day'].values)}\n")
