...
def analyze_snowfall_diagnostics(nc_dir, diag_csv, output_csv, output_txt):
    ...
    with open(output_txt, "w") as f:
        ...
        if diag_cols:
            ...
            for col in diag_cols:
                if snow_df[col].notna().any():
                    f.write(
                        f"  {col}: mean={snow_df[col].mean():.3e}, min={snow_df[col].min():.3e}, max={snow_df[col].max():.3e}\n"
                    )
        ...
        for _, row in snow_df.iterrows():
            f.write(f"\nDay {int(row['day']):2d}:\n")
            f.write(
                f"  ERA5 sf rate: {row['sf_rate_mean']:.3e} m/s (max {row['sf_rate_max']:.3e})\n"
            )
            if not np.isnan(row["snow_depth_mean"]):
                f.write(
                    f"  Snow depth: {row['snow_depth_mean']:.4f} m (max {row['snow_depth_max']:.4f})\n"
                )
            if not np.isnan(row["ice_thick_mean"]):
                f.write(
                    f"  Ice thick: {row['ice_thick_mean']:.3f} m (max {row['ice_thick_max']:.3f})\n"
                )
            if not np.isnan(row["ice_conc_mean"]):
                f.write(f"  Ice conc: {row['ice_conc_mean']:.3f}\n")
            f.write(
                f"  T_surf: {row['temp_surf_mean']:.2f} °C (range {row['temp_surf_min']:.2f}..{row['temp_surf_max']:.2f})\n"
            )
            f.write(
                f"  T_air: {row['air_temp_mean']:.2f} °C (range {row['air_temp_min']:.2f}..{row['air_temp_max']:.2f})\n"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze snowfall diagnostics from model output"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-csv", default="data/output/snowfall_daily.csv", help="Output CSV"
    )
    parser.add_argument(
        "--output-txt",
        default="data/output/snowfall_report.txt",
        help="Output report TXT",
    )
    args = parser.parse_args()

    return analyze_snowfall_diagnostics(
