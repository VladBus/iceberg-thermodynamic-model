...
def parse_args():
    ...
    parser.add_argument("--run-id", required=True, help="Run ID (e.g., 2020_Q1_snowfall_ERA5_5day)")
    parser.add_argument("--day-start", type=int, default=1, help="Start day (default: 1)")
    parser.add_argument("--day-end", type=int, default=None, help="End day (default: all available)")
    parser.add_argument("--output-dir", default=None, help="Output directory (default: run figures dir)")
...
def compute_snowfall_stats(ds):
    ...
    stats['sf_global_cv'] = float(np.nanstd(sf) / np.nanmean(sf)) if np.nanmean(sf) > 0 else 0
    stats['sf_water_cv'] = float(np.nanstd(sf[water]) / np.nanmean(sf[water])) if np.nanmean(sf[water]) > 0 else 0
    stats['sf_ice_cv'] = float(np.nanstd(sf[ice_mask]) / np.nanmean(sf[ice_mask])) if np.nanmean(sf[ice_mask]) > 0 else 0
...
def compute_correlation(ds):
    ...
    if np.sum(valid) > 10:
        results['corr_water'] = float(np.corrcoef(sf_water[valid], sd_water[valid])[0, 1])
...
def main():
    ...
    print("=" * 100)
    print(f"{'Day':>4} {'SF_in(m3/d)':>14} {'SF_in_ice(m3/d)':>16} {'Storage(m3)':>14} {'dStorage(m3)':>14} {'Residual(m3)':>14} {'Closure':>8} {'Corr_ice':>10}")
    print("=" * 100)
    ...
    for day, f in day_files:
        ...
        closure = delta_storage / stats['sf_volume_total'] if stats['sf_volume_total'] > 0 else 0
        
        corr_ice = corr.get('corr_ice', np.nan)
        
        print(f"{day:4d} {stats['sf_volume_total']:14.2e} {stats['sf_volume_ice']:16.2e} {storage:14.2e} {delta_storage:14.2e} {residual:14.2e} {closure:8.2f} {corr_ice:10.4f}")
    ...
    for k in range(5):
        print(f"  Cat {k}: mean_sd={np.nanmean(sd[k]):.4e}, max_sd={np.nanmax(sd[k]):.4f}, "
