...
ROUND_TRIPS = [
    (
        ...
        units.temperature_c_to_k,
        np.array([173.15, 253.15, 273.15, 293.15, 320.0]),
    ),
    (
        "velocity m/s <-> cm/s",
        units.velocity_mps_to_cmps,
        units.velocity_cmps_to_mps,
        np.array([0.0, 1e-3, 1.0, 25.0, 60.0]),
    ),
    (
        "stress Pa <-> dyn/cm2",
        units.stress_pa_to_dyncm2,
        units.stress_dyncm2_to_pa,
        np.array([0.0, 0.01, 0.1, 1.0, 5.0]),
    ),
    (
        "dp Pa/m <-> hPa/km",
        units.dp_pam_to_hpakm,
        units.dp_hpakm_to_pam,
        np.array([0.0, 1e-4, 1e-3, 0.1, 2.0]),
    ),
    (
        "pressure Pa <-> hPa",
        units.pressure_pa_to_hpa,
        units.pressure_hpa_to_pa,
        np.array([50000.0, 95000.0, 101325.0, 110000.0]),
    ),
    (
        "density_anomaly kg/m3 <-> g/cm3",
        units.density_anomaly_kgm3_to_gcm3,
        units.density_anomaly_gcm3_to_kgm3,
...
def test_temperature_offset_identity():
    ...
    np.testing.assert_allclose(
        units.temperature_c_to_k(units.temperature_k_to_c(t_k)),
        t_k,
        rtol=RTOL,
        atol=0.0,
    )
