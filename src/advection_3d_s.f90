...
! Физика: Решает уравнение конвекции-диффузии: ∂S/∂t + U·∇S = 0.
...
!         Схема идентична advection_3d_t:
...
!           4. FCT-лимитер (Zalesak) для каждого направления — ограничение потоков
!              для сохранения монотонности (нет новых экстремумов).
!           5. Коррекция — обновление поля солености путем вычитания
!              ограниченных антидиффузионных потоков.
! Единицы: S [массовая доля]; U/V [см/с]; W [см/с]; dx [см]; dz [см]; dt [с]; c2 = dt/dx.
! Ответственность: Транспортировка солености течениями с гарантиями сохранения
!                  строгой положительности концентрации и массы. Исключает
!                  возникновение нефизичных отрицательных значений.
! ==============================================================================
    ...
    subroutine advs(dt, c2)
        ...
        do j = 1, js
            ...
            do i = 1, is
                ...
                do k = 1, ki
                    cc = s2(i, j, k)              ! Значение в текущей ячейке
                    ci1 = s2(i1, j, k)            ! Сосед по +X
                    ci2 = s2(i2, j, k)            ! Сосед по -X
                    cj1 = s2(i, j1, k)            ! Сосед по +Y
                    cj2 = s2(i, j2, k)            ! Сосед по -Y
                ...
                do k = 1, ki - 1
                    ...
                    a = abs(ww)
                    ck1 = s2(i, j, k1)
                    ! Трехдиагональная схема: CD = TT - dt*(W*dS/dz)
                    cd(i, j, k) = tt(k) - dt* &
        ...
        ! Финальное обновление массива солености S2 = CD
        s2(:, :, :) = cd(:, :, :)
