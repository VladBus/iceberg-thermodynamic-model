    ...
    subroutine advsh(dt1)
        ...
        do j = 3, js2
            do i = 2, is2
                ...
                ! В оригинальном Advsh.f90 в X-блоке поперечный поток CDY
                ! игнорируется (умножен на 0), в Y-блоке учитывается полностью.
                cd2(i, j) = uij - c10*0.5*(cdx + 0.0*cdy)

                ! Антидиффузионные потоки FCT.
                ! В оригинальном Advsh.f90 промежуточные переменные (UJP, UU, UJ1,
                ! UI1, UIP, VIP) в X-блоке НЕ инициализируются, поэтому равны нулю и
                ! антидиффузионные потоки APX2/APY2 здесь нулевые (эквивалент флагу
        ...
        do j = 2, js2
            do i = 3, is2
                ...
                ! Антидиффузионные потоки FCT.
                ! В оригинальном Advsh.f90 промежуточные переменные (VJP, VIP,
                ! UJP, VV, VJ1, VI2) в Y-блоке НЕ инициализируются, поэтому равны
                ! нулю и антидиффузионные потоки APX2/APY2 здесь нулевые (см.
                ! комментарий в X-блоке).
                vv = 0.0
                vj1 = 0.0
                vi2 = 0.0
                vjp = 0.0
