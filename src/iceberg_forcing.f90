...
contains
    ...
    ! ========================================================================
    ! Конвенция индексов (Stage 7.6A, src/grid_coupling.f90, model_coords_to_indices):
    !   i — модельный Y-индекс (север/широта-подобный);
    !   j — модельный X-индекс (восток/долгота-подобный);
    !   wx — вес вдоль X, интерполирует ВДОЛЬ j (wx=0 у j1, wx=1 у j2);
    !   wy — вес вдоль Y, интерполирует ВДОЛЬ i (wy=0 у i1, wy=1 у i2).
    !
    ! Формула (Stage 10.15.2 fix — перекрёстные члены исправлены; ранее веса
    ! были транспонированы: wx применялся к i, wy к j → разрывы ~1 ячейки
    ! при пересечении границ ячеек, см. docs/validation/stage10.15.1_*):
    !   f = wx1*wy1*f(i1,j1) + wx*wy1*f(i1,j2)
    !     + wx1*wy*f(i2,j1) + wx*wy*f(i2,j2)
    !
    ...
    !
    ! Конвенция индексов (Stage 7.6A, src/grid_coupling.f90):
    !   i — модельный Y-индекс (север/широта-подобный);
    !   j — модельный X-индекс (восток/долгота-подобный);
    !   wx — вес вдоль X, интерполирует ВДОЛЬ j;
    !   wy — вес вдоль Y, интерполирует ВДОЛЬ i.
    !
    ! Формула (Stage 10.15.2 fix — перекрёстные члены исправлены):
    ...
    subroutine model_coords_to_latlon(x_model, y_model, lat, lon, ok)
        ...
        ! wx интерполирует вдоль j (X), wy вдоль i (Y).
