...
999     continue
    ...
    ! Формула:
    !   sec = days * 86400 + hours * 3600
    ! где days — число дней от 1970-01-01.
    ! 86400 = 24*3600 — секунд в сутках.
    ! 3600 = 60*60 — секунд в часе.
    ...
    ! При выходе за диапазон idx прижимается к границе (extrapolation prohibited).
    ! Сложность: O(log N), где N = era5_ntime (до 124 для месяца, до ~2200 для года).
    ...
    !
    ! Формула билинейной интерполяции на регулярной сетке:
    !   value = (1-wlat)*(1-wlon)*f[ilat,jlon] + wlat*(1-wlon)*f[ilat1,jlon]
    !         + (1-wlat)*wlon*f[ilat,jlon1]    + wlat*wlon*f[ilat1,jlon1]
    ! где:
    !   wlat = (lat - lat0)/(lat1 - lat0) — вес по широте [0..1]
    !   wlon = (lon - lon0)/(lon1 - lon0) — вес по долготе [0..1]
    !   ilat/jlon — индексы нижнего узла, ilat1/jlon1 — верхнего.
    !
    ! Особенности:
    !   - ERA5 longitude циклическая: jlon1 оборачивается (jlon+1 → 1 на dateline).
    !   - latitude предполагается монотонно возрастающей (переворачивается при чтении).
    !   - Индексы узлов ищутся бинарным поиском (шаг сетки может отличаться от 0.25°).
