...
contains
    ...
    subroutine write_nc(filename)
        ...
        ! Формула: x(i) = (i-1) * 13.89 [км], y(j) = (j-1) * 13.89 [км]
        do i = 1, is1
            x_coord(i) = real(i - 1)*13.89  ! [км]
        end do
        do i = 1, js1
            y_coord(i) = real(i - 1)*13.89  ! [км]
        end do
        ! depth = центры Z-уровней (реальный массив z из param.f90) [см] → [м]
        depth = z*1.0e-2
        ! depth_w = границы Z-уровней (для W на ks1 узлах):
        !   depth_w(1) = 0.0 — поверхность
        !   depth_w(k) = 0.5*(z(k-1)+z(k)) — середина между соседними центрами [м]
        !   depth_w(ks1) = z(ks) + 0.5*(z(ks)-z(ks-1)) — экстраполяция нижней границы
        depth_w(1) = 0.0
        do k = 2, ks
...
status = nf90_put_att(ncid, nf90_global, 'unit_system', 'SI (canonical external units, Stage 5.5b)')
  ...
  status = nf90_def_var(ncid, 'density_anomaly', nf90_real, (/x_dimid, y_dimid, z_dimid/), ro_varid)
     ...
     'ice area fraction [0-1] for 5 thickness categories (internal fraction)')
      ...
      call set_att(ncid, dpy_varid, 'comment', 'canonical SI unit Pa m-1 (internal hPa km-1 * 0.1)')
       ...
       call set_att(ncid, tatm_varid, 'comment', 'canonical SI unit K; internal model unit is degC')
        ...
        !   T [°C] → temperature [K]: +273.15
        !   RO [г/см³] → density_anomaly [кг/м³]: ×1000 (аномалия rho-1.02, НЕ абсолютная)
        !   U/V/W [см/с] → [м/с]: ×0.01
        !   windx/windy [см/с] → [м/с]: ×0.01
        !   tx/ty [дин/см²] → tau [Па]: ×0.1 (1 Па = 10 дин/см²)
        !   dpx/dpy [гПа/км] → dp [Па/м]: ×0.1 (1 гПа/км = 0.1 Па/м)
        !   tatm [°C] → air_temp [K]: +273.15
        !   patm [гПа] → air_press [Па]: ×100
        !   s2 [массовая доля] → salinity_mass_fraction [1]: без изменений (SI canonical)
        t_k = t2 + 273.15
        ro_kgm3 = ro*1000.0
        u_ms = u2*0.01
        v_ms = v2*0.01
