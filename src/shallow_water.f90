...
!         Уравнения мелкой воды: ∂η/∂t + ∇·(H·U) = 0; ∂U/∂t + g·∇η = -f×U + τ/ρH.
!         Схема Лейса-Рихтмайера с FCT-коррекцией для импульса.
! Единицы: U/V [см/с], η [см], H [см], dt1 [с], g [см/с²], f [1/с].
...
module shallow_water
    ...
    ! JJQ - счетчик баротропных шагов для приливных частот.
    ! EUU - кинетическая энергия для диагностики.
...
contains
    ...
    subroutine shal()
        ...
        ! dt1 = 120 с (баротропный шаг), dx = 1389000 см (~13.89 км).
        ! mm3 = 30 баротропных шагов за один бароклинный (30×120 = 3600 с).
        ! c1 = g = 981 см/с² (гравитация).
        ! c3 = A_h/dx² = 7.5e6/1389000² ≈ 3.9e-6 [1/с] (горизонтальная вязкость).
        ! c9 = 0.5/dx [1/см] (градиентный множитель).
        ! c10 = dt1/dx [с/см] (число Куранта для уровня).
        ! aa1 = g/dx [1/с²] (градиент давления).
        ! aa2 = 0.0026·dt1 [см] (коэффициент трения о дно).
        ! aa3 = 0.0055·dt1 [см] (коэффициент трения льда о воду).
        dt1 = 120.0
        dx = 1389000.0
        mm3 = 30
        c1 = 981.0/1.0
        ...
        ! --- ОСНОВНОЙ БАРОТРОПНЫЙ ЦИКЛ (MM3 = 30 шагов) ---
        do jjj = 1, mm3
            jjq = jjq + 1
            ...
            ! Расчет уровня моря Y (уравнение неразрывности: ∂η/∂t + ∇·(H·U) = 0)
            do j = 2, js
                do i = 2, is
            ...
            ! --- РАСЧЕТ U-КОМПОНЕНТЫ (интегральный поток UP2) ---
            do j = 2, js
                j1 = j + 1
                j2 = j - 1
                do i = 2, is
            ...
            ! --- РАСЧЕТ V-КОМПОНЕНТЫ (интегральный поток VP2) ---
            do j = 2, js
                j1 = j + 1
                j2 = j - 1
                do i = 2, is
            ...
            ! Шаг баротропного расчета течений (FCT схема для импульса)
            call advsh(dt1)
