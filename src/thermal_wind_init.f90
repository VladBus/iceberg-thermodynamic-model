...
module thermal_wind_init
    ...
    integer, parameter :: INIT_DISCRETE_SS = 5
...
contains
    ...
    subroutine init_thermal_wind()
        ...
        case (INIT_DISCRETE_SS)
            ! Дискретное состояние равновесия Block 200
            print *, ">>> Discrete steady-state initialization (Block 200 exact)"
            call compute_discrete_steady_state(0.05, 0.02)

            ! Инициализация SSH (y2=0 для дискретного равновесия)
            y2 = 0.0
            YM2 = 0.0
    ...
    subroutine compute_discrete_steady_state(u_ref, v_ref)
        ...
        do j = 2, JS
            do i = 2, IS
                ...
                f_val = FKU(i, j)
                if (abs(f_val) .lt. 1e-12) then
                    ! У экватора — ставим нулевую скорость
                    do k = 1, ki
                        U2(i, j, k) = 0.0
                        V2(i, j, k) = 0.0
                    end do
        ...
        ! Диагностика
        call diagnose_thermal_wind()
    end subroutine compute_discrete_steady_state
    ! ==========================================================================
