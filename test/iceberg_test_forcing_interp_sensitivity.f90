...
contains
    ...
    subroutine init_synthetic_forcing(ocean_prof_out, atmos_out)
        ...
        do k_local = 1, nlevels
            ...
            ocean_prof_out%u(k_local) = 0.1*sin(real(k_local)/10.0)
            ocean_prof_out%v(k_local) = 0.05*cos(real(k_local)/10.0)
        end do
    ...
    subroutine get_synthetic_ocean_bilinear(x, y, ocean_prof_out)
        ...
        do k_local = 1, nlevels
            ...
            ocean_prof_out%temp(k_local) = -1.0
            ocean_prof_out%salt(k_local) = 0.034
            ocean_prof_out%u(k_local) = 0.1*sin(x/1e6 + real(k_local)/10.0)
            ocean_prof_out%v(k_local) = 0.05*cos(y/1e6 + real(k_local)/10.0)
        end do
